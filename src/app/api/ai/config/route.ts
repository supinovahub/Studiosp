  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.json(
        { error: 'Falha ao carregar a configuração do AI' },
        { status: 500 }
      );
    }
...
export async function POST(request: Request) {
  try {
    ...
    if (!Number.isFinite(maxPer)) maxPer = 30;
    maxPer = Math.min(50, Math.max(10, Math.floor(maxPer)));
    ...
    if (existing) {
      ...
      if (upErr) {
        ...
        return NextResponse.json(
          { error: 'Falha ao salvar a configuração do AI' },
          { status: 500 }
        );
      }
    } else {
...
export async function DELETE() {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json(
        { error: 'Falha ao excluir a configuração do AI' },
        { status: 500 }
      );
    }
    return NextResponse.json({ success: true });

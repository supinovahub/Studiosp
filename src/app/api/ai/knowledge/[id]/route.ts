  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.json(
        { error: 'Falha ao carregar o documento' },
        { status: 500 }
      );
    }
    if (!data)
    ...
    if (content !== undefined) {
      ...
      if (corrupt) {
        return NextResponse.json({
          success: true,
          warning:
            'Atualizado apenas com pesquisa por palavra-chave — sua chave de incorporação não pôde ser descriptografada (marque ENCRYPTION_KEY e digite a chave novamente).',
        });
      }
    }

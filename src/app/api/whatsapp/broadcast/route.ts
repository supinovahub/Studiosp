...
import { sendProviderTemplate } from '@/lib/whatsapp/provider';
  ...
  try {
    ...
    if (Array.isArray(newRecipients) && newRecipients.length > 0) {
      ...
      return NextResponse.json(
        {
          error:
            'Forneça `destinatários` (preferencial) ou `números_telefone` — deve ser uma matriz não vazia',
        },
        { status: 400 }
      );
    }
    ...
    if (!template_name) {
      return NextResponse.json(
        { error: 'nome_modelo é obrigatório' },
        { status: 400 }
      );
    }
    ...
    if (rawTemplateRow && !isMessageTemplate(rawTemplateRow)) {
      return NextResponse.json(
        {
          error:
            'A linha do modelo está malformada localmente – execute "Sync from Meta" em Configurações para repará-la antes da transmissão.',
        },
        { status: 500 }
      );
    }
    ...
    for (const recipient of recipients) {
      ...
      const variants =
        config.provider === 'uazapi' ? [sanitized] : phoneVariants(sanitized);

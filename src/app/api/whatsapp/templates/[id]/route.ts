  ...
  try {
    ...
    if (!existing.meta_template_id) {
      return NextResponse.json(
        {
          error:
            'Este modelo nunca foi enviado ao Meta – use Novo modelo para enviá-lo.',
        },
        { status: 400 }
      );
    }
    ...
    if (payload.category === 'Authentication') {
      return NextResponse.json(
        {
          error:
            'Os modelos de AUTENTICAÇÃO não são editáveis ​​aqui – gerencie-os no Meta WhatsApp Manager.',
        },
        { status: 400 }
      );
    }
  ...
  try {
    ...
    if (existing.meta_template_id && !isDryRun()) {
      ...
      if (configError || !config || !config.waba_id) {
        return NextResponse.json(
          {
            error: 'WhatsApp não configurado – não é possível excluir no Meta.',
          },
          { status: 400 }
        );
      }

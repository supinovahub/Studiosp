...
describe('mondayIndex', () => {
  it('maps Monday → 0 and Sunday → 6', () => {
    expect(mondayIndex(new Date(2026, 4, 18))).toBe(0); // Segunda
    expect(mondayIndex(new Date(2026, 4, 19))).toBe(1); // Terça
  ...
  it('aligns with DOW_SHORT_MON_FIRST labels', () => {
    expect(DOW_SHORT_MON_FIRST[mondayIndex(new Date(2026, 4, 18))]).toBe('Seg');
    expect(DOW_SHORT_MON_FIRST[mondayIndex(new Date(2026, 4, 24))]).toBe('Dom');
  });
});

...
export async function sendMediaMessage(
  ...
  if (!link) throw new Error('sendMediaMessage requer um link.');
  const url = `${META_API_BASE}/${phoneNumberId}/messages`;
...
export async function uploadResumableMedia(
  ...
  if (!startData.id) {
    throw new Error('O upload recuperável não retornou um ID de sessão.');
  }
  ...
  if (!uploadData.h) {
    throw new Error(
      'O upload recuperável não retornou um identificador de arquivo.'
    );
  }
...
export async function submitMessageTemplate(
  ...
  const data = await response.json();
  if (!data?.id) {
    throw new Error('Meta aceitou o modelo, mas não retornou nenhum ID.');
  }
  return {
    id: String(data.id),
...
export async function sendInteractiveButtons(
  ...
  for (const btn of buttons) {
    if (!btn.id) throw new Error('Botão interativo sem ID.');
...
export async function sendInteractiveList(
  ...
  validateInteractiveHeaderFooter(headerText, footerText);
  if (!buttonLabel)
    throw new Error('A lista interativa requer um buttonLabel.');
  ...
  for (const section of sections) {
    for (const row of section.rows) {
      if (!row.id) throw new Error('Linha da lista interativa sem ID.');
...
function validateInteractiveBody(bodyText: string): void {
  if (!bodyText) throw new Error('A mensagem interativa requer bodyText.');
...
export async function getMediaUrl(
  ...
  const data = await response.json();
  if (!data.url)
    throw new Error('URL de mídia não encontrado na resposta Meta');
  return {
    url: data.url,

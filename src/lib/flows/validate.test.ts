...
describe('validateFlowForActivation — flow-level', () => {
  ...
  it('flags empty node list', () => {
    ...
    expect(issues.some((i) => i.message.includes('pelo menos um nó'))).toBe(
      true
    );
  });
  ...
  it('flags duplicate node_key', () => {
    ...
    expect(
      issues.some(
        (i) =>
          i.message.includes('chave da etapa está duplicada') &&
          i.node_key === 'a'
      )
    ).toBe(true);
  });
  ...
  it('flags keyword trigger with no keywords', () => {
    ...
    expect(
      issues.some(
        (i) =>
          i.scope === 'trigger' &&
          i.message.includes('pelo menos uma palavra-chave')
      )
    ).toBe(true);
  });
...
describe('validateFlowForActivation — nodes', () => {
    ...
    expect(
      issues.some(
        (i) =>
          i.node_key === 'b' &&
          i.field === 'buttons' &&
          i.message.includes('pelo menos um')
      )
    ).toBe(true);
  });
  ...
  it('flags duplicate button reply_ids', () => {
    ...
    expect(
      issues.some((i) =>
        i.message.includes('ID de resposta do botão está duplicado')
      )
    ).toBe(true);
  });
  ...
  it('flags list row title over 24 chars', () => {
    ...
    expect(issues.some((i) => i.message.includes('excede 24 caracteres'))).toBe(
      true
    );
  });
    ...
    expect(
      issues.some(
        (i) =>
          i.node_key === 'orphan' &&
          i.severity === 'warning' &&
          i.message.includes('não pode ser alcançada')
      )
    ).toBe(true);
  });
  ...
  it("doesn't crash on unknown node_type — flags it", () => {
    ...
    expect(
      issues.some((i) => i.message.includes('Tipo de etapa desconhecido'))
    ).toBe(true);
  });
});
